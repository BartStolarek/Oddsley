...
class TaskRegistryTests(TestCase):
    ...
    def test_execute_hello_world_task(self):
        ...
        self.assertEqual(captured_output.getvalue().strip(), "Hello World task executed")
    ...
    def test_execute_parameterized_task(self):
        ...
        sys.stdout = sys.__stdout__  # Reset redirect.
        self.assertEqual(captured_output.getvalue().strip(), "Parameterized task executed with value1 and value2")

    def test_get_unregistered_task(self):
        with self.assertRaises(ValueError) as context:
            TaskRegistry.get_task("non_existent_task")
        
        self.assertEqual(str(context.exception), "Task non_existent_task not found in registry or as importable string")
