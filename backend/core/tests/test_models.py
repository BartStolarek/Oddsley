...
from core.models import Sport, Competition, Team, Event

class SportModelTest(TestCase):
    def setUp(self):
        Sport.objects.create(key="football", group="Ball Sports", title="Football", active=True)
...
class CompetitionModelTest(TestCase):
    def setUp(self):
        sport = Sport.objects.create(key="football", group="Ball Sports", title="Football", active=True)
